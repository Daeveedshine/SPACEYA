...
const SplashScreen: React.FC = () => (
  <div className="fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center text-white transition-opacity duration-1000 overflow-hidden">
    ...
    >
      <source
        src="https://assets.mixkit.co/videos/preview/mixkit-drone-view-of-a-mansion-with-a-pool-and-garden-4286-large.mp4"
        type="video/mp4"
      />
    </video>
...
const App: React.FC = () => {
  ...
  const refreshBadges = () => {
    ...
    const notifications = store.notifications.filter(
      (n) => n.userId === u.id && !n.isRead,
    ).length;

    let maintenance = 0;
    if (u.role === UserRole.AGENT) {
      const agentProps = store.properties
        .filter((p) => p.agentId === u.id)
        .map((p) => p.id);
      maintenance = store.tickets.filter(
        (t) =>
          t.status === TicketStatus.OPEN && agentProps.includes(t.propertyId),
      ).length;
    ...
    let screenings = 0;
    if (u.role === UserRole.AGENT) {
      screenings = store.applications.filter(
        (a) => a.status === ApplicationStatus.PENDING && a.agentId === u.id,
      ).length;
    } else if (u.role === UserRole.ADMIN) {
      screenings = store.applications.filter(
  ...
  const navItems = [
    ...
    },
    {
      ...
      label: "Overview",
      icon: Home,
      roles: [UserRole.AGENT, UserRole.TENANT],
    },
    {
      id: "applications",
      label: "Apply Now",
    ...
    },
    {
      ...
      label: "Properties",
      icon: Building2,
      roles: [UserRole.AGENT, UserRole.ADMIN, UserRole.TENANT],
    },
    {
      id: "maintenance",
      label: "Maintenance",
  ...
  return (
    <div className="flex flex-col md:flex-row h-[100dvh] text-zinc-900 dark:text-white transition-colors duration-300 overflow-hidden relative">
      ...
      >
        <div
          className={`p-6 md:p-8 h-full flex flex-col ${isSidebarCollapsed ? "items-center" : ""}`}
        >
          <div
            className={`mb-8 md:mb-10 ${isSidebarCollapsed ? "text-center" : ""}`}
          >
            <div
              className={`inline-block bg-blue-600/10 dark:bg-blue-400/10 p-4 rounded-[1.5rem] border border-blue-600/20 transition-all ${isSidebarCollapsed ? "p-3" : "md:p-5"}`}
            >
              <Logo
                size={isSidebarCollapsed ? 28 : 36}
          ...
          <nav className="space-y-1 flex-1 overflow-y-auto custom-scrollbar pr-1">
            {navItems
              .filter((item) =>
                item.roles.includes(user?.role || UserRole.TENANT),
              )
              .map((item) => (
                <button
                  key={item.id}
                ...
                >
                  <item.icon
                    className={`${isSidebarCollapsed ? "" : "mr-4"} h-5 w-5 shrink-0 ${view === item.id ? "text-white" : "text-zinc-400 group-hover:text-blue-600"}`}
                  />
                  {!isSidebarCollapsed && (
          ...
          >
            ...
            >
              {theme === "light" ? (
                <Moon
                  className={`${isSidebarCollapsed ? "" : "mr-4"} h-5 w-5`}
                />
              ) : (
                <Sun
                  className={`${isSidebarCollapsed ? "" : "mr-4"} h-5 w-5`}
